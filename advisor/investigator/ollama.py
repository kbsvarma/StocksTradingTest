...
def generate(prompt, schema, config, timeout, post=None, output_tokens=8192):
    ...
    if len(prompt) > 52000:
        raise ValueError('Local research context exceeds 52,000 characters; select evidence before inference')
    body = {'model': model, 'stream': True, 'think': thinking, 'truncate': False, 'shift': False, 'format': schema,
            'messages': [{'role': 'system', 'content': SYSTEM}, {'role': 'user', 'content': prompt}],
            'options': {'num_ctx': 32768, 'num_predict': min(output_tokens, 8192 if thinking else 4096),
    ...
    return result, {'provider': 'ollama', 'model': model, 'elapsed_seconds': round(time.monotonic()-started, 2),
                    ...
                    'context_characters': len(prompt), 'context_tokens': 32768,
