...
def packet(ticker,rows,analysis):
    ...
    for r in chosen:
        docs.append({'source_id':'S'+str(len(docs)+1),'url':r['url'],'title':r.get('title'),'period_end':r.get('period_end'),
            ...
            'text':passages(r['payload']['text'],6500,priority_terms=('estimated useful lives','finance to operating','excluding OpenAI','reportable segments','Net cash from operations','Additions to property',
...
def generate(ticker,rows,analysis,*,post=None,config=None,trace=None):
    ...
    refs=set(re.findall(r'\[S(\d+)\]',value['report_markdown']))
    if any(int(n)<1 or int(n)>len(context['documents']) for n in refs):raise ValueError('Unknown source reference')
    value['report_markdown']=re.sub(r'\[S(\d+)\]',lambda m:'[Source '+m[1]+']('+context['documents'][int(m[1])-1]['url']+')',value['report_markdown'])
    ...
    value['report_markdown']=re.sub(r'\[(https?://[^\]\s]+)\](?!\()',lambda m:'[Source]('+m[1]+')',value['report_markdown'])
